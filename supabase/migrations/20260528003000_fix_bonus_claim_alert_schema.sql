-- =============================================================
-- Fix: claim_bonus usa el esquema real de server_alerts
-- =============================================================
-- server_alerts no tiene status/details; usa resolved, player_id y metadata.
-- La referencia anterior rompia el reclamo con "column status does not exist".
...
BEGIN
  ...
  SELECT EXISTS (
    ...
    WHERE category = 'collusion'
      ...
      AND (
        player_id = v_user_id
        OR metadata->>'player_1' = v_user_id::text
        OR metadata->>'player_2' = v_user_id::text
      )
