...
- El script raíz conserva `test:handles` como diagnóstico explícito y no debe ejecutarse en cada commit: Jest documenta que `--detectOpenHandles` implica ejecución serial y tiene penalización significativa. `--openHandlesTimeout=5000` queda en las corridas normales.
...
- La gracia oficial de reconexión es de `60s`; los tests de `ConnectionManager.ts` deben cubrir éxito antes del límite y expulsión después del límite.
