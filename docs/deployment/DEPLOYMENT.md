...
> `APP_URL` debe apuntar al dominio web publico que recibira los enlaces firmados del panel admin (recuperacion de contraseña y cambio de correo). Ese mismo origen debe existir en la allowlist de redirects de Supabase Auth.
...
### Broadcast & Push (Required for notifications)

```env
# Frontend (Vercel) — VAPID public key for push subscriptions
NEXT_PUBLIC_VAPID_PUBLIC_KEY="tu-clave-publica-vapid"

# Frontend (Vercel) — game server connection for broadcast dispatch
GAME_SERVER_URL="https://game-server.tu-dominio.com"
INTERNAL_API_SECRET="secreto-compartido-entre-web-y-game-server"

# Game Server — VAPID keys for Web Push
VAPID_PUBLIC_KEY="tu-clave-publica-vapid"
VAPID_PRIVATE_KEY="tu-clave-privada-vapid"
VAPID_SUBJECT="mailto:admin@tu-dominio.com"
```

> **Note:** The game server must have Redis running for BullMQ push queue.
> The push worker (`push.worker.ts`) starts automatically with the game server.
