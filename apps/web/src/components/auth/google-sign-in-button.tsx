...
export function GoogleSignInButton({ label = 'Continuar con Google' }: { label?: string }) {
  ...
  async function handleClick() {
    ...
    // If no error, the browser is redirecting to Google
  }
  ...
  return (
    <div className="w-full">
      <button
        ...
        onClick={handleClick}
        disabled={loading}
        className="w-full h-14 flex items-center justify-center gap-3 bg-white/5 hover:bg-white/10 border-2 border-white/10 hover:border-white/20 rounded-2xl text-text-premium font-bold text-base transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed shadow-inner"
      >
        {loading ? (
