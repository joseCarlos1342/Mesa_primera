  ...
  return (
    <div className="min-h-screen p-4 md:p-8 max-w-5xl mx-auto">
      ...
      <div className="mb-10 text-center md:text-left">
        <h1 className="text-3xl md:text-5xl font-black italic tracking-tighter text-[var(--accent-gold)] flex items-center justify-center md:justify-start gap-3 drop-shadow-[0_4px_12px_rgba(234,179,8,0.2)]">
        ...
        <p className="text-slate-300 font-medium mt-2 text-sm md:text-base bg-black/30 inline-block px-4 py-1.5 rounded-full border border-white/5 backdrop-blur-sm">
          <span className="text-[var(--accent-gold)] font-bold">{replays.length}</span> {replays.length === 1 ? 'partida' : 'partidas'} en la mesa <span className="font-mono text-[var(--accent-gold)]/70 pl-1">{roomId.substring(0, 8)}</span>
        </p>
      </div>

      {/* Replays List */}
      {replays.length === 0 ? (
        <div className="text-center py-24 bg-black/20 rounded-[2rem] border border-white/5 backdrop-blur-md">
          <Film className="w-20 h-20 text-[var(--accent-gold)]/40 mx-auto mb-6" />
        ...
        <div className="grid gap-4 md:gap-5 grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
          {replays.map((r) => {
            ...
            return (
              ...
              >
                {/* Result Accent Line */}
                <div className={`absolute top-0 left-0 right-0 h-1.5 ${r.is_winner ? 'bg-gradient-to-r from-emerald-500/50 to-emerald-400' : 'bg-gradient-to-r from-red-500/50 to-red-400'}`} />
                ...
                <div className="flex flex-col h-full justify-between gap-5 mt-1">
                  ...
                  <div className="flex justify-between items-start">
                    ...
                    <div className="text-right">
                      <p className="text-[10px] font-black text-[var(--accent-gold)] uppercase tracking-widest flex items-center justify-end gap-1 mb-0.5">
                  ...
                  {playerNames.length > 0 && (
                    <div className="py-3 border-y border-white/5">
                      <p className="text-[10px] font-black text-[var(--accent-gold)] uppercase tracking-widest mb-1.5 flex items-center gap-1">
