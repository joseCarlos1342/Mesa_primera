...
export async function getWalletData() {
  ...
  // 2. Get Realized Ledger Entries (bóveda: depósitos, retiros, reembolsos, ajustes, transferencias, bonos)
  const { data: ledger, error: ledgerError } = await supabase
    .from('ledger')
    .select('*')
    .eq('user_id', user.id)
    .in('type', ['deposit', 'withdrawal', 'refund', 'adjustment', 'admin_adjustment', 'transfer', 'bonus'])
